export const HOSTING_CONFIG_KEY = "archigen_hosting_config";
...
/*This function is used to generate a unique identifier (slug)—usually for things that need a distinct name in URLs, storage, or databases.
Naming uploaded floor plans --> Users may upload with same file names (image.png), You need no collisions*/

export const createHostingSlug = () =>
    `archigen-${Date.now().toString(36)}-${Math.random()
        .toString(36)
        .slice(2, 8)}`;
